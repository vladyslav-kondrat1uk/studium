  ...
  uploadImg() {
    console.log("dwdqwdqwdq");
      ...
      if (files.length > 0) {
          ...
          console.log(imgLinkElement);
          console.log(fieldsetElement);
          console.log(btShowFileUploadElement);

          console.log("dewfwfewfw");
...
processArtEditFrmData(event) {
  ...
  processArtInsertFromData(event) {
      event.preventDefault();
      console.log("test");
      const articleData = {
        title: this.formElements.namedItem("title").value.trim(),
      ...
      fetch(`${this.serverUrl}/article/`, postReqSettings)
        ...
        .then(() => {
          window.alert("Article was successfully posted");
        })
        .catch((error) => {
          window.alert(`Failed to post. ${error}`);
        })
        .finally(() => (window.location.hash = `#articles/1/${this.totalCount}`));
    }
}
